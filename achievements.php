...
foreach ($member_list as $member)
{
	...
	$games_points_pre = 0;
	$sql =<<< EOT
SELECT J.*, $names_for_query FROM journal AS J 
	...
	while($r = $adapter->fetchArray(SQLITE3_ASSOC))
	{
		...
		if($first_row)
		{
			...
			for($i=0; $i<count($names); $i++)
			{
				...
				if(isset($r[$name]))	$values[$name] = $r[$name];
				else			$values[$name] = "";
			}
			...
			for($i=0; $i<count($names); $i++)
			{
				...
				$index = $i + 1;
				$rows[$index] = "<td>$names[$name]</td> <td class='full'>$values[$name]</td>";
			}
		...
		else
		{
			...
			for($i=0; $i<count($names); $i++)
			{
				...
				$index = $i + 1;
				$rows[$index] .= "<td class='full'>$values[$name]</td>";
			}
